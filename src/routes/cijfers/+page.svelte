<script lang="ts">
	...
	if ($subjectGrades.length === 0 || $mostGrades === 0) {
		const subjects = getSubjects(data.grades).sort();
...
<table class="text-lg border-hidden rounded-md shadow-zinc-900 shadow-xl bg-zinc-500">
	<thead class="h-10">
			...
			<tr class="h-cell">
				...
				{#each Array($mostGrades) as _, i}
					{#if subjectGrades.grades[i]}
						<td class="p-0 border-zinc-600/30 border"
					...
					{:else}
						<td class="w-cell h-cell border-zinc-600/30 border" />
					{/if}
				{/each}
				<td class="p-0 w-2cell h-cell border-zinc-600/30 border-t"
					><p class="flex justify-center items-center">
